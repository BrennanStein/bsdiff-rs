pub use super::rust::bsdiff_raw as bsdiff_raw;
pub use super::rust::bspatch_raw as bspatch_raw;
use std::io::{Read, Write};

use rand::Rng;

type BsDiffFn = fn(old: &[u8], new: &[u8], patch: &mut dyn Write) -> Result<(), i32>;
type BsPatchFn = fn(old: &[u8], new: &mut [u8], patch: &mut dyn Read) -> Result<(), i32>;

macro_rules! backend_tests {
        ($d:ident, $p:ident) => {
            use crate::raw::{hardcoded_data_t, long_random_data_t};
            #[test]
            fn hardcoded_data() {
